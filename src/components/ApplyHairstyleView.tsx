...
const ApplyHairstyleView: React.FC<ApplyHairstyleViewProps> = ({ hairstyle, user }) => {
    ...
    const handleApplyHairstyle = useCallback(async () => {
        if (!uploadedImage) {
        ...
        try {
            // Use "hairstyle" as the style/prompt context. 
            // We might want to pass explicit prompt if we had one, but using the gender and name helps.
    ...
    }, [uploadedImage, hairstyle.previewImageUrl, hairstyle.name, hairstyle.gender]);
        ...
        <div className="max-w-2xl mx-auto animate-fade-in flex flex-col gap-6 p-4">
            ...
            <div className="bg-base-200 dark:bg-dark-base-200 p-4 sm:p-6 rounded-lg shadow-xl">
                <h1 className="text-2xl sm:text-3xl font-bold text-content-100 dark:text-dark-content-100 mb-2">
                    {hairstyle.name}
                    <span className="text-lg font-light ml-2 capitalize">({hairstyle.gender})</span>
                </h1>
                {hairstyle.description && <p className="text-content-200 dark:text-dark-content-200 mb-4">{hairstyle.description}</p>}
                ...
                <div className="aspect-video w-full bg-base-300 dark:bg-dark-base-300 rounded-lg flex items-center justify-center overflow-hidden mb-4">
                    <img src={hairstyle.previewImageUrl} alt="Hairstyle Preview" className="object-contain max-h-full max-w-full" />
                </div>
                ...
                >
                    {isLoading ? 'Applying...' : 'Apply Hairstyle'}
                </button>
            </div>
            ...
            {(isLoading || generatedImage) && (
                <div className="bg-base-200 dark:bg-dark-base-200 p-4 sm:p-6 rounded-lg shadow-xl">
                    <h2 className="text-xl font-semibold text-content-100 dark:text-dark-content-100 mb-4 text-center">Your New Look</h2>
                    <div className="min-h-[250px] w-full flex items-center justify-center">
                        {isLoading ? (
                            ...
                            <img src={generatedImage} alt="Generated result" className="max-w-full max-h-[50vh] object-contain rounded-lg shadow-lg" />
                        )}
                    </div>
...
export default ApplyHairstyleView;
