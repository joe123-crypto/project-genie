...
import { Template, ViewState, User } from '@/types';
...
interface CreateTemplateViewProps {
  ...
  onUpdateTemplate?: (templateToUpdate: Template) => Promise<void> | void;
  onBack?: () => void;
}
...
const CreateTemplateView: React.FC<CreateTemplateViewProps> = ({
  ...
  const handleGenerateImage = async () => {
    ...
    try {
      const img = await generateImage(formData.prompt, 'templates');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (templateToEdit && onUpdateTemplate) {
        await onUpdateTemplate({ ...templateToEdit, ...payload });
      } else {
        const saved = await saveTemplate(payload);
        if (addTemplate) addTemplate(saved);
      }
